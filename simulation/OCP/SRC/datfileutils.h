...
inline std::vector<double> datfile_get_vector (const std::string &filename, std::string id, int stage_id = -1, int node_id = -1);
...
inline std::vector<double> datfile_get_vector(const std::string &filename, std::string id, int stage_id, int node_id) {
...
inline bool run_external_command (const char* command) {
	...
	if (status != 0) {
        std::cerr << "[run_external_command] failed!" << std::endl;
		return false;
	}
